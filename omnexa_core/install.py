...
def _conf_flag(name: str) -> bool | None:
	try:
...
def _auto_discover_github_apps_enabled() -> bool:
	conf = _conf_flag("omnexa_install_all_github_apps")
	if conf is not None:
		return conf
	return str(os.environ.get("OMNEXA_INSTALL_ALL_GITHUB_APPS", "0")).strip().lower() in (
		"1",
		"true",
		"yes",
		"on",
...
def _auto_install_full_stack_on_core_enabled() -> bool:
	"""Install only the core/basic stack by default — not ``sites/apps.txt`` / org discovery.

	Set ``OMNEXA_AUTO_INSTALL_FULL_STACK_ON_CORE=1`` or site config
	``omnexa_auto_install_full_stack_on_core: 1`` for legacy full-bench bootstrap.
	"""
	conf = _conf_flag("omnexa_auto_install_full_stack_on_core")
	if conf is not None:
		return conf
...
def run_site_hardening_after_app_changes():
	...
	try:
		...
		try:
			from omnexa_core.omnexa_core.industry_solutions.industry_solutions_workspace import sync_industry_solutions_home

			sync_industry_solutions_home()
		except Exception:
			frappe.log_error(frappe.get_traceback(), "Omnexa: sync_industry_solutions_home")
		try:
...
def ensure_company_activity_select_options():
	...
	try:
		...
		for fieldname in ("business_activity", "industry_sector", "production_demo_activity"):
			name = frappe.db.get_value("DocField", {"parent": "Company", "fieldname": fieldname
	}, "name")
			if not name:
				continue
...
def ensure_dashboard_compliance_cards():
	...
	try:
		...
		def upsert_card(label: str, filters: list[tuple[str, str, str]]):
			...
			card_name = frappe.db.get_value("Number Card", {"label": label
	}, "name")
			if card_name:
...
def ensure_dashboard_finance_cards():
	...
	try:
		...
		if frappe.db.exists("DocType", "Bank Statement Line"):
			...
			bank_card = frappe.db.get_value("Number Card", {"label": "Bank Unmatched Statement Lines"
	}, "name")
			if bank_card:
...
def ensure_finance_workflow_templates():
	...
	try:
		...
		def _has_existing(doc_type: str) -> bool:
			try:
				return bool(frappe.db.get_value("Workflow", {"document_type": doc_type, "is_active": 1
	}, "name"))
			except Exception:
				return False
...
def export_bench_app_git_heads():
	"""Return ``{app: {"head": "<sha>", "dirty": bool}
	}`` from each ``apps/<app>`` git checkout.
...
def apply_default_branding():
	...
	try:
		...
		if app_logo_svg_path.exists():
			...
			for old_file in frappe.get_all("File", filters={"file_url": logo_file_url
	}, pluck="name"):
				try:
		...
		# 2) Optional compatibility: older setups where logo.svg existed in bench root.
			...
			# 3) Legacy PNG fallback (previous requirement).
				...
				for old_file in frappe.get_all("File", filters={"file_url": legacy_png_file_url
	}, pluck="name"):
					try:
						frappe.delete_doc("File", old_file, force=1, ignore_permissions=True)
					except Exception:
...
def _grant_full_access_to_setup_user(args: dict) -> None:
	...
	try:
		...
		if company:
			head_branch = frappe.db.get_value("Branch", {"company": company, "is_head_office": 1
	}, "name")
			if head_branch:
...
def _ensure_branch(company, branch_name, branch_code, tax_id):
	...
	existing_head_office = frappe.db.get_value("Branch", {"company": company, "is_head_office": 1
	}, "name")
	...
	existing_same_code = frappe.db.get_value("Branch", {"company": company, "branch_code": branch_code
	}, "name")
