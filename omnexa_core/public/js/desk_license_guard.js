...
// DISABLED: Allow access without license for development/testing
(function () {
	"use strict";

	// License guard disabled - allow all access without license
	return;
	...
	function blockWithMessage(app) {
		...
		let lockMsg = "";
		if (warnings) {
			warningMsg = __("Warnings: {0}. ", [warnings]);
		}
		if (lockAt) {
			lockMsg = __("Lock at (UTC ts): {0}. ", [lockAt]);
		}
		const message = __("The application {0} is not licensed (status: {1}). {2}{3}Open ErpGenEx Marketplace and enter a valid license or developer key.", [app, st, warningMsg, lockMsg]);
		frappe.msgprint({
			title: __("License required"),
			indicator: "red",
			message: message,
		});
		const actionMessage = __('<a href="{0}">Open Marketplace</a>', [mp]);
		frappe.msgprint({
			title: __("Action"),
			indicator: "blue",
