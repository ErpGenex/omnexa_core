...
def _doctype_inventory(module: str) -> list[str]:
	return frappe.get_all("DocType", filters={"module": module, "istable": 0
...
def audit_erpgenex_verticals() -> dict[str, Any]:
	...
	for key, block in modules.items():
		...
		block["workspace"] = frappe.db.get_value("Workspace", {"module": mod
	}, "name") or ""
		block["workspace_links"] = frappe.db.count("Workspace Link", {"parent": block["workspace"]
	}) if block["workspace"] else 0
