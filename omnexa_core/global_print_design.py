...
def _ensure_default_print_format_for_doctype(doctype: str) -> None:
	...
	existing = frappe.db.get_value("Print Format", {"name": format_name
	}, "name")
	if not existing:
		doc = frappe.get_doc(
			{
...
def ensure_default_letter_head() -> str | None:
	...
	name = frappe.db.get_value("Letter Head", {"letter_head_name": GLOBAL_LETTER_HEAD_NAME
	}, "name")
	if name:
...
def ensure_global_print_design_system() -> None:
	...
	style_name = frappe.db.get_value("Print Style", {"print_style_name": GLOBAL_PRINT_STYLE_NAME
	}, "name")
	if style_name:
