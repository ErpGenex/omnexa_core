...
def _trim_non_head_branches(company: str) -> list[str]:
	...
	for branch in frappe.get_all(
		"Branch", filters={"company": company, "is_head_office": 0
...
def rebuild_activity_demo_companies(
	...
	for activity, ar_label, abbr in ACTIVITY_DEMO_SPECS:
		if frappe.db.exists("Company", {"abbr": abbr
			...
			deleted_companies.append(_delete_company_fully(frappe.db.get_value("Company", {"abbr": abbr
	}, "name")))
		try:
	...
	head_branch = frappe.db.get_value("Branch", {"company": main, "is_head_office": 1
	}, "name")
	if head_branch:
