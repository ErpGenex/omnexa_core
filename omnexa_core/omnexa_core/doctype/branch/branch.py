...
class Branch(Document):
	...
	def _validate_country_code(self):
		...
		# Extract country code from full format (e.g., "EG — Egypt" -> "EG")
		if " — " in country_code:
			country_code = country_code.split(" — ")[0].strip()
			# Update the field to store only the code for consistency
	...
	@frappe.whitelist()
	def branch_demo_action_healthcare_masters(self):
		from omnexa_healthcare.demo_procedure_data import create_all_demo_data

		try:
			result = create_all_demo_data(company=self.company, branch=self.name)
			frappe.msgprint(
				_("Healthcare masters demo seeded: specialties, procedures, and equipment created for branch {0}.").format(self.name),
				title=_("Healthcare Masters Demo"),
				indicator="green",
			)
			...
			frappe.msgprint(
				_("Error creating healthcare masters demo: {0}").format(str(e)),
				title=_("Error"),
				indicator="red",
			)
	...
	@frappe.whitelist()
	def branch_demo_action_healthcare_procedures_consumables(self, force=0):
		from omnexa_healthcare.demo_procedure_data import create_all_demo_data

		try:
			# If force is enabled, delete existing demo data first
			if cint(force):
				self._delete_healthcare_procedures_demo_data()
			
			result = create_all_demo_data(company=self.company, branch=self.name)
			frappe.msgprint(
				_("Healthcare procedures and consumables demo seeded for branch {0}.").format(self.name),
				title=_("Healthcare Procedures & Consumables Demo"),
				indicator="green",
			)
			return {"success": True, "result": result
	}
			...
			frappe.msgprint(
				_("Error creating healthcare procedures and consumables demo: {0}").format(str(e)),
				title=_("Error"),
				indicator="red",
			)
			return {"success": False, "error": str(e)
	...
	def _delete_healthcare_procedures_demo_data(self):
		"""Delete existing healthcare procedures demo data"""
		from omnexa_healthcare.utils.branch_demo_seed import DEMO_MARKER
		
		# Delete demo procedures
		procedures = frappe.get_all("Healthcare Procedure", {"procedure_code": ["like", f"{DEMO_MARKER}%"]}, pluck="name")
		for proc in procedures:
			try:
				frappe.delete_doc("Healthcare Procedure", proc, force=1, ignore_permissions=True)
			except Exception:
				pass
		
		# Delete demo specialties (only those created by demo)
		demo_specialty_codes = ["CAR", "ORT", "GEN", "DER", "OBG", "PED", "DENTAL", "NEURO", "GAST", "UROL"]
		for code in demo_specialty_codes:
			if frappe.db.exists("Healthcare Specialty", code):
				try:
					frappe.delete_doc("Healthcare Specialty", code, force=1, ignore_permissions=True)
				except Exception:
					pass
		
		# Delete demo equipment
		equipment = frappe.get_all("Healthcare Medical Device", {"device_code": ["like", "HMD-%"]}, pluck="name")
		for eq in equipment:
			try:
				frappe.delete_doc("Healthcare Medical Device", eq, force=1, ignore_permissions=True)
			except Exception:
				pass
	...
	@frappe.whitelist()
	def branch_demo_action_execute(self):
		demo_type = self.get("branch_demo_type")
		if not demo_type:
			frappe.throw(_("Please select a demo type first."), title=_("Validation"))

		# Map demo type to action
		action_map = {
			"Branch Masters Demo": "seed_masters",
			"Branch Masters + Transactions Demo": "seed_with_tx",
			"6-Month Business Simulation": "seed_6m",
			"12-Month Business Simulation": "seed_12m",
			"Construction Portfolio Demo": "construction_portfolio",
			"Healthcare Hospital Demo (20 patients + web bookings)": "healthcare_hospital",
			"Healthcare Masters Demo (specialties + procedures + equipment)": "healthcare_masters",
			"Healthcare Procedures & Consumables Demo": "healthcare_procedures_consumables",
			"Finance Group Demo (50 clients × all apps)": "finance_group",
			"EduSphere Education Demo": "education",
			"Hotel Assets Demo (50 assets)": "hotel_assets"
		}

		action = action_map.get(demo_type)
		if not action:
			frappe.throw(_("Invalid demo type selected."), title=_("Validation"))

		# Call the appropriate action
		action_method = getattr(self, f"branch_demo_action_{action}", None)
		if not action_method:
			frappe.throw(_("Demo action not found: {0}").format(action), title=_("Error"))
