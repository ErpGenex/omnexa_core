...
_APP_SPECS: dict[str, dict[str, Any]] = {
	...
	"omnexa_einvoice": {
		...
		"status_doctypes": ["Tax Authority Profile"],
		"kpis": [
			("E-Invoice Submissions", "E Invoice Submission", []),
			("Tax Submission Logs", "Country Tax Submission Log", []),
			("ZATCA Logs", "ZATCA Submission Log", []),
			("Tax Profiles", "Tax Authority Profile", []),
			("Signing Profiles", "Signing Profile", []),
			("Country Tax Settings", "Country Tax Settings", []),
		],
		"shortcuts": [
			("ETA Console", "Page", "eta-einvoice-console"),
			("E-Invoice Submission", "DocType", "E Invoice Submission"),
			("Tax Authority Profile", "DocType", "Tax Authority Profile"),
			("E-Invoice Review Queue", "Report", "ETA E-Invoice Review Queue"),
			("Governance Overview", "Report", "Governance Overview"),
		],
		"kpi_trends": [
			{"type": "Bar", "doctype": "E Invoice Submission", "group_by": "status", "label": "Submission status"
	},
			{"type": "Pie", "doctype": "Country Tax Submission Log", "group_by": "status", "label": "Tax log outcomes"
	},
		],
		"extra_sections": [
			(
				"Tax consoles",
				[
					("ETA E-Receipt Console", "Page", "eta-ereceipt-console", "receipt"),
					("International Tax Console", "Page", "international-tax-console", "globe"),
					("ZATCA Console", "Page", "zatca-console", "shield"),
				],
			),
		]},
	"omnexa_alm": {
	...
	"omnexa_hr": {
		...
		"status_doctypes": ["HR Employee Appraisal", "HR Interview", "HR Payroll Entry"],
		"kpis": [
			("Employees", "Employee", []),
			("Attendance Rows", "HR Attendance", []),
			("Expense Claims", "HR Expense Claim", []),
			("Leave Applications", "HR Leave Application", []),
			("Appraisals", "HR Employee Appraisal", []),
		...
		"shortcuts": [
			...
			("Expense claims", "DocType", "HR Expense Claim"),
			...
			("Appraisals", "DocType", "HR Employee Appraisal"),
	...
	"omnexa_fixed_assets_insurance": {
		"_requires_app": "omnexa_fixed_assets",
		"workspace": "Asset Insurance",
		"module": "Omnexa Fixed Assets",
		"icon": "shield",
...
def prune_invalid_workspace_kpi_artifacts() -> None:
	...
	singles = frappe.get_all("DocType", filters={"issingle": 1
	}, pluck="name")
	virtual = frappe.get_all("DocType", filters={"is_virtual": 1
	}, pluck="name")
...
def _build_content(
	...
	blocks.append(
		{
			...
			"type": "header",
			"data": {"text": f"<span class=\"h4\"><b>{spec['headline']
	}</b></span>", "col": 12}
	}
	)
		...
		blocks.append(
			{
				...
				"type": "header",
				"data": {"text": f"<span class=\"h5\"><b>{title
	}</b></span>", "col": 12}
	}
		)
		op_idx += 1
		for item in items:
			blocks.append({"id": f"{slug}-op{op_idx
	}", "type": "shortcut", "data": {"shortcut_name": item, "col": op_col}
	})
			op_idx += 1
	...
	if grouped_ops:
		for sc in other_ops:
			blocks.append({"id": f"{slug}-op{op_idx
	}", "type": "shortcut", "data": {"shortcut_name": sc, "col": op_col}
	})
			op_idx += 1
	else:
	...
	for i, sc in enumerate(report_shortcuts[:18]):
		blocks.append({"id": f"{slug}-rp{i
	}", "type": "shortcut", "data": {"shortcut_name": sc, "col": 4}
	})
	...
	for i, ch_name in enumerate(chart_full_names):
		blocks.append({"id": f"{slug}-ch{i
	}", "type": "chart", "data": {"chart_name": ch_name, "col": 4}
	})
...
def _append_module_reports_to_shortcut_seed(shortcut_seed: list[Any], module: str, minimum_reports: int = 3) -> None:
	...
	if report_count >= minimum_reports:
		return
	seen = {_shortcut_seed_key(s) for s in shortcut_seed if _shortcut_seed_key(s)}
	for name in frappe.get_all("Report", filters={"module": mod
	}, pluck="name", order_by="name asc", limit=24):
		if report_count >= minimum_reports:
			break
...
def _build_engineering_v1_workspace_bundle(
	...
	if onb and frappe.db.exists("Module Onboarding", onb):
		blocks.append(
			{"id": f"{slug
	}-onb", "type": "onboarding", "data": {"onboarding_name": onb, "col": 12}
	},
		)
	blocks.append(
		{
			...
			"type": "header",
			"data": {"text": f"<span class=\"h4\"><b>{headline
	}</b></span>", "col": 12}
	},
	)
	blocks.append({"id": f"{slug
	}-oph", "type": "header", "data": {"text": "<b>📌 Operations</b>", "col": 12}
	})
	op_i = 0
	for row in shortcut_rows:
		...
		if not nm:
			continue
		blocks.append({"id": f"{slug}-op{op_i
	}", "type": "shortcut", "data": {"shortcut_name": nm, "col": 4}
	})
		op_i += 1
		if op_i >= 24:
			break
	blocks.append({"id": f"{slug
	}-rph", "type": "header", "data": {"text": "<b>📊 Reports</b>", "col": 12}
	})
	rp_i = 0
	for row in shortcut_rows:
		if (row.get("type") or "") != "Report":
		...
		if not nm:
			continue
		blocks.append({"id": f"{slug}-rp{rp_i
	}", "type": "shortcut", "data": {"shortcut_name": nm, "col": 4}
	})
		rp_i += 1
		if rp_i >= 12:
			break
	blocks.append({"id": f"{slug
	}-kph", "type": "header", "data": {"text": "<b>📊 KPIs</b>", "col": 12}
	})
	for i, lbl in enumerate(number_card_labels[:4]):
		blocks.append({"id": f"{slug}-nc{i
	}", "type": "number_card", "data": {"number_card_name": lbl, "col": 4}
	})
	if chart_names:
		blocks.append({"id": f"{slug
	}-chh", "type": "header", "data": {"text": "<b>📈 Charts</b>", "col": 12}
	})
		for i, ch in enumerate(chart_names[:3]):
			blocks.append({"id": f"{slug}-ch{i
	}", "type": "chart", "data": {"chart_name": ch, "col": 4}
	})
	blocks.append(
		{
			"id": f"{slug
	...
	if desk:
		for ci, (card_title, _rows) in enumerate(desk):
			...
			if not ttl:
				continue
			blocks.append({"id": f"{slug}-c{ci
	}", "type": "card", "data": {"card_name": ttl, "col": 4}
	})
...
_VERTICAL_APP_OWNED_WORKSPACES: dict[str, str] = {
	...
	"projects": "omnexa_projects_pm",
	"Trading": "omnexa_trading",
	"Manufacturing": "omnexa_manufacturing",
	"Tourism": "omnexa_tourism",
	"engineering-consulting": "omnexa_engineering_consulting",
	"Restaurant": "omnexa_restaurant",
	"Services": "omnexa_services",
	"Agriculture": "omnexa_agriculture",
	"HR": "omnexa_hr",
	"Accounting": "omnexa_accounting",
	"Nursery": "omnexa_nursery",
	"Fixed Assets": "omnexa_fixed_assets",
	...
	"RE Development": "erpgenex_realestate_dev",
	"RE Marketing": "erpgenex_realestate_sales",
