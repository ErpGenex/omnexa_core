	...
	*,
	...
	for row in link_rows:
		if row.get("type") == "Card Break":
			if section_idx:
				content.append({"id": f"{slug}-sp-{section_idx}", "type": "spacer", "data": {"col": 12}})
			content.append(
				{
	...
	if ws.number_cards:
		content.append({"id": f"{slug}-kpi-sp", "type": "spacer", "data": {"col": 12}})
		content.append(
			{
				"id": f"{slug}-kpi-h",
				"type": "header",
	...
	if ws.charts:
		content.append({"id": f"{slug}-ch-sp", "type": "spacer", "data": {"col": 12}})
		content.append(
			{
				"id": f"{slug}-ch-h",
				"type": "header",
