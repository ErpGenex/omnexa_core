...
def _write_live_snapshot(closure: dict) -> None:
	...
	payload = {
		...
		"closure": {
			...
			"uat_passed": (closure.get("uat") or {
	}).get("scenarios_passed"),
			"uat_total": (closure.get("uat") or {
	}).get("scenarios_total"),
			"wave5_passed": (closure.get("wave5") or {
	}).get("connectors_passed"),
			"gaps_ok": (closure.get("gaps") or {}).get("ok")
	},
		"benchmark": closure.get("benchmark"),
		"gaps": closure.get("gaps"),
		"uat": {"passed": (closure.get("uat") or {
	}).get("scenarios_passed"), "total": (closure.get("uat") or {}).get("scenarios_total")
	},
