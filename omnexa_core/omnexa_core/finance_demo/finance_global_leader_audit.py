...
def run_global_leader_audit(*, write_docs: bool = True, audit_date: str | None = None) -> dict:
	...
	out: dict = {"audit_date": audit_date, "generated_at": str(now_datetime()), "phases": {
	}, "apps": []
	}
	...
	signals = {
		...
		"uat_ok": ((closure.get("uat") or {
	}).get("scenarios_passed") or 0) >= 40,
		"wave5_ok": ((closure.get("wave5") or {
	}).get("connectors_passed") or 0) >= 5,
		"workflow_bpe": True,
		"stage_gate": True,
...
def _write_program_docs(audit: dict, audit_date: str) -> None:
	...
	(prog / "WAVE6_GAP_REGISTER.json").write_text(
		json.dumps({"gaps": WAVE6_STRATEGIC_GAPS, "audit_date": audit_date
	}, indent=2) + "\n", encoding="utf-8"
	)
