...
def _uninstall_protected_apps() -> frozenset[str]:
	...
	Also includes basic platform apps unless ``omnexa_marketplace_allow_uninstall_basic`` is set.
	"""
	from omnexa_core.omnexa_core.marketplace_install import basic_platform_apps

	out = {"frappe", "omnexa_core"}
	if not _is_truthy(frappe.conf.get("omnexa_marketplace_allow_uninstall_basic")):
		out.update(basic_platform_apps())
...
def _install_app_if_needed(
	...
	try:
		...
		return {
			...
			"install_source": source,
			"install_ref": ref,
			"install_policy": "basic_only" if marketplace_basic_deps_only_enabled() else "full_required_apps",
			"skipped_required_apps": skipped_required_apps_for(app_slug),
		...
		return {"installed": False, "message": "install_failed", "error": str(exc)
	}
...
def get_install_plan(app_slug: str):
	...
	if local_available:
		try:
			...
			install_refs = []
	missing_basic = missing_basic_platform_apps(exclude=app_slug)
	skipped_required = skipped_required_apps_for(app_slug)
	basic_only = marketplace_basic_deps_only_enabled()
	policy_note = (
		frappe._(
			"Only core and basic platform apps are installed automatically as prerequisites. "
			"Other declared dependencies are not pulled unless you install them separately."
		)
		if basic_only
		else frappe._("Full required_apps chain from app hooks will be installed.")
	)
	return {
		"app_slug": app_slug,
		"repo_url": _approved_repo_for_app(app_slug),
		"install_sources": [
			{
				"value": "local",
				"label": "Local server copy (recommended)",
				"enabled": local_available,
				"note": "Use app already present on this bench server (fastest, no get-app).",
			},
			{
				"value": "github",
		...
		"quick_install_available": local_available,
		...
		"install_policy": "basic_only" if basic_only else "full_required_apps",
		"install_policy_note": policy_note,
		"missing_basic_apps": missing_basic,
		"skipped_required_apps": skipped_required
	}
